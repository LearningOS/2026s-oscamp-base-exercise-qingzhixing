...
impl TaskContext {
    ...
    pub fn init(&mut self, stack_top: usize, entry: usize) {
        let aligned_stack_top = (stack_top + 15) & (!15);
...
pub unsafe extern "C" fn switch_context(_old: &mut TaskContext, _new: &TaskContext) {
    ...
    core::arch::naked_asm!(
        ...
        "ld s10,96(a1)",
        "ld s11,104(a1)",
        //clear a0,a1 to prevent pointer leak
        "xor a0,a0,a0",
        "xor a1,a1,a1",
        //ret
        "ret",
...
pub fn alloc_stack() -> (Vec<u8>, usize) {
    let extra = 15;
    let buffer = vec![0u8; STACK_SIZE + extra];
    let base = buffer.as_ptr() as usize;
    let top = (base + STACK_SIZE + 15) & !15;
    (buffer, top)
}
