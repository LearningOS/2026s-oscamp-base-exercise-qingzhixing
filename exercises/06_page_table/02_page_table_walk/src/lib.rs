...
use crate::TranslateResult::{PageFault, PermissionDenied};
...
impl SingleLevelPageTable {
    ...
    pub fn translate(&self, va: u32, is_write: bool) -> TranslateResult {
        ...
        let vpn = (va >> PAGE_OFFSET_BITS) as usize;
        let offset = va & ((1 << PAGE_OFFSET_BITS) - 1);
        if let Some(pte) = self.entries[vpn] {
            let ppn = pte.ppn;
            let flags = pte.flags;
            if (flags & PTE_VALID) == 0 {
                return PageFault;
            }
            if is_write && ((flags & PTE_WRITE) == 0) {
                return PermissionDenied;
            }
            return TranslateResult::Ok(ppn * (PAGE_SIZE as u32) + offset);
        } else {
            return PageFault;
        }
    }
